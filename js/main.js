...
countries.forEach((country) => {
  $(`<option>`, { text: country.name, val: country.dial_code }).appendTo($select);
}); 
...
/*scroll-animation*/
  ...
  $(function(){
    $('#selectCountry').on('change',function(){
      ...
      $('#code').val(this.value);
      const maskPhone = (e) => {
          const regexp = new RegExp(`(\\+)(\\d{0,${this.value.length - 1}})(\\d{0,3})(\\d{0,7})`)
          var x = e.target.value.replace(/[\(\)\s-]/g, '').match(regexp);
          e.target.value = !x[2] ? x[1] : x[1] +  x[2]  + ( !x[3] ? '' : (x[4] ? ' (' + x[3] + ') ': ' (' + x[3] )  + (x[4] ? (x[4].length < 4 ? x[4] : x[4].slice(0,3) + ' - ' + x[4].slice(3)) : '') )
        }
      document.getElementById('code').removeEventListener('input', maskPhone);
      document.getElementById('code').addEventListener('input', maskPhone);
    });
  });
...
// document.querySelector('form').addEventListener('submit', e => {
  ...
  $(".sign-up-button").on('click', function() {
    ...
    var myPhone = $(".phone-number").val().trim().replace(/\W/g, '');
    var myCountry = document.querySelector('.current').innerText;
    ...
    formdata.append("lname", [mySurame]);
    ...
    fetch("https://api.topmediagroups.com/api/v1/users", requestOptions).then((data) => {
      if (data.status === 201) {
        login()
      }
    })
    function login() {
      ...
      loginData.append('password', [myPass])
      loginData.append('email', [myEmail])
      let token;
    ...
    function tasks(token) {
      let taskData = new FormData()
        taskData.append('name', [myName, mySurame])
        taskData.append('description', 'я успешно прошел тестовое задание')
        fetch('https://api.topmediagroups.com/api/v1/tasks', {
          ...
          document.location.href = document.location.origin + '/thanks';
    }
  });
